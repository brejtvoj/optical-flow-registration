...
def multiscale_ssim_loss(image1, image2, flow, patch_size=3, n_levels=4):
    """
    Calculate the SSIM loss at multiple resolutions.
    ...
    -------
    loss : Tensor
        Comulative loss of the SSIM loss taken at differing resolutions.
    """
    gamma = 0.4
    loss = 0.0
    w = 0.0
    ...
    for i in range(n_levels):
        ...
        wi = gamma**(n_levels - i)
        w += wi
        loss_i = wi * weighted_ssim(image1, image2)
        loss += loss_i
    loss /= w
    return loss